...
open_db();
if(!mysqli_query($db,"UPDATE service_ticket_tab SET service_ticket_status = 5,service_ticket_cdate = '$today' WHERE service_ticket_id = $service_ticket_id"))
{
	?>
...
else
{
	//pull more details on new ticket
		$category_res = mysqli_query($db,"SELECT sttt.service_ticket_type,l.`name`,stt.owner_fName,stt.owner_lName,stt.owner_tel,stt.owner_email,stt.service_ticket_desc,stt.short_desc 
										FROM service_ticket_tab stt, service_ticket_type_tab sttt,location l WHERE stt.service_ticket_type_id = sttt.service_ticket_type_id AND stt.location = l.ID 
										AND stt.service_ticket_id = $service_ticket_id");
		$category_row = mysqli_fetch_array($category_res);
				
		$notice = "<b>".getUserFullName(getCurrentUsername())."</b> is UNABLE TO RESOLVE Service Ticket - # $service_ticket_id <br><br><b>Ticket Details: </b><br>".$category_row['service_ticket_desc'];
		...
		if(!addNotification("Administrator Cannot Solve Ticket  - # ".$service_ticket_id." (".$category_row['service_ticket_type']." - ".$category_row['short_desc'].")",$notice,11,$owners,0,1,$category_row['owner_email'],NULL))
		{
